...
export default function NewProducts({ onAddToCart }: NewProductsProps) {
  ...
  const Card = ({ item }: { item: (typeof newItems)[0] }) => (
    <div className="relative bg-[#1a1a1a] border border-white/6 rounded-2xl overflow-hidden group h-full">
      <div className="absolute top-2.5 right-2.5 z-10 flex items-center gap-1 bg-[#FDE080] text-[#0a0a0a] text-[10px] font-bold px-2.5 py-1 rounded-full shadow-md">
      ...
      <div className="p-4">
        ...
        <p className="text-gray-600 text-xs line-clamp-2 mb-3 leading-relaxed">{item.description}</p>
        <div className="flex items-center justify-between gap-2">
          <span className="text-[#FDE080] font-bold text-sm shrink-0">{formatPrice(item.price)}</span>
          <motion.button
            onClick={() => onAddToCart(item, item.category)}
            className="flex items-center gap-1.5 bg-[#FDE080] text-[#0a0a0a] px-3 py-2 rounded-full text-xs font-semibold hover:bg-[#e8cc6a] transition-colors"
            whileHover={btnHover}
            whileTap={btnTap}
          >
  ...
  return (
    <section className="py-14 bg-[#0a0a0a]">
      <div className="container mx-auto px-4">
        ...
        >
          <div>
            <span className="inline-flex items-center gap-1.5 text-[#FDE080] font-medium uppercase tracking-widest text-xs mb-1">
              <Sparkles className="w-3.5 h-3.5" />
              {t.newProducts.badge}
            </span>
            <h2 className="text-2xl md:text-3xl font-bold text-white font-serif">{t.newProducts.title}</h2>
          </div>
          <motion.button
            onClick={() => document.querySelector("#menu")?.scrollIntoView({ behavior: "smooth" })}
            className="hidden sm:flex items-center gap-1.5 text-sm font-medium text-[#FDE080]/70 hover:text-[#FDE080] transition-colors"
            whileHover={{ x: 3 }}
          >
            {t.nav.menu} →
          </motion.button>
            ...
            {newItems.map((_, i) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${
                  i === activeIndex ? "w-7 bg-[#FDE080]" : "w-2 bg-white/15"
                }`}
              />
            ))}
          </div>
